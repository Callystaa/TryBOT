...
async def fban(event):
    ...
    if len((fed_list := get_flist())) == 0:
        return await event.edit("**Anda belum terhubung ke federasi manapun!**")
    ...
    if failed:
        ...
        status = f"Sukses! Fbanned di {total} feds."
...
async def delf(event):
    ...
    del_flist_all()
    await event.edit("**Memutuskan koneksi dari semua federasi yang terhubung!**")


CMD_HELP.update(
    {
        "fban": ">`.fban <id/nama pengguna> <alasan>`"
        "\nUsage: Ban pengguna dari federasi yang terhubung."
        "\nAnda dapat membalas ke pengguna yang ingin Anda fban atau secara manual memasukkan nama pengguna/id."
        "\n\n`>.unfban <id/nama pengguna> <alasan>`"
        "\nUsage: Sama seperti fban tetapi unban pengguna"
        "\n\n>`.addf <nama>`"
        "\nUsage: Menambahkan grup saat ini dan menyimpannya sebagai <nama> dalam federasi yang terhubung."
        "\nMenambahkan satu grup sudah cukup untuk satu federasi."
        "\n\n>`.delf`"
        "\nUsage: Menghapus grup saat ini dari federasi yang terhubung."
        "\n\n>`.listf`"
        "\nUsage: Mencantumkan semua federasi yang terhubung dengan nama yang ditentukan."
        "\n\n>`.clearf`"
        "\nUsage: Memutuskan koneksi dari semua federasi yang terhubung. Gunakan dengan hati-hati."
    }
)
